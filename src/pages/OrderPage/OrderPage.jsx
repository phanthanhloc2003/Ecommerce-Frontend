...
function OrderPage() {
  ...
  const handleDeleteProduct = (idProduct) =>
    ...
    useEffect(() => {
      const sum = order.filter(item => checkBox.includes(item.product))
                      .reduce((acc, arr) => acc + arr.many, 0);
                      setMany(sum)
    }, [checkBox , order]);

// console.log(order.filter(item  => checkBox.includes(item.product)))
  ...
  return (
    <div className="bg-[#F5F5FA]">
      <div className="container p-[24px]">
        ...
        {order.length <= 0 ? <CartEmpty/> : (<div className="flex flex-nowrap justify-between">
                ...
                <div className="py-[17px] px-[20px] flex flex-nowrap justify-between m-0">
                  ...
                  <div className="text-right ">
                    <div className="text-[#FE3734] text-[15px] font-normal text-right ">
                       ...
                       {many ?   currency(many, { symbol: "", precision: 0 })
                              .format()
                              .replace(/,/g, "."):  "   Vui lòng chọn sản phẩm" }
                    </div>
