...
function UpdateAddress() {
  ...
  useEffect(() => {
    const fetcDetailsAddress = async () => {
      const data = await getDetailsAddress(id , user.access_Token);
      setName(data?.name);
      ...
      setSelectedWard(data?.selectedWard);
      setValue(data?.value);
      setIsCheck(data?.ischeck);
      setIsChecks(data?.ischeck)
      ...
      <div className="">
        ...
        <div className="bg-[#fff] rounded-[4px] min-h-[400px] py-[30px] px-[20px]">
          <form className="w-[600px]" onSubmit={handleSubmit}>
            ...
            {!isChecks &&  <div className="flex items-center mb-[15px]">
